...
class ControllerModuleJournal2SuperFilter extends Controller {
    ...
    protected function getFilters($module_id, $ajax = FALSE) {
        ...
        if ($ajax) {
            $this->data['full_path'] = Journal2Utils::getProperty($this->request->post, 'full_path');
    ...
    private function parseUrl(){
        ...
        if (isset($this->request->post['search']) && strlen($this->request->post['search']) > 0) {
            ...
            $data['tags'][] = $this->request->post['search'];
        }
        ...
        } else if (isset($this->request->post['full_path'])) {
            $data['full_path'] = $this->request->post['full_path'];
        } else {
            $data['full_path'] = '';
        }
    ...
    public function products() {
        ...
        foreach ($results as $result) {
            ...
            $this->data['products'][] = array(
                ...
                'thumb2'      => $image2,
                'name'        => $result['name'],
                'description' => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, Front::$IS_OC2 ? (version_compare(VERSION, '2.2', '<') ? $this->config->get('config_product_description_length') : $this->config->get($this->config->get('config_theme') . '_product_description_length')) : 100 . '..'),
                'price'       => $price,
                'special'     => $special,
                'date_end'    => $date_end,
                'tax'         => $tax,
